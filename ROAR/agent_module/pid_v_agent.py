...
from ROAR.control_module.pid_v_controller import PIDVController
...
class PIDVAgent(Agent):
    def __init__(self, target_speed=40, **kwargs):
        super().__init__(**kwargs)
        self.target_speed = target_speed
        self.logger = logging.getLogger("PID Agent")
        self.route_file_path = Path(self.agent_settings.waypoint_file_path)
        self.pid_controller = PIDVController(agent=self, steering_boundary=(-1, 1), throttle_boundary=(0, 1))
    ...
    def run_step(self, vehicle: Vehicle,
        ...
        super(PIDVAgent, self).run_step(vehicle=vehicle,
